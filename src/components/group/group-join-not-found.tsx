...
interface GroupJoinNotFoundProps {
  token?: string;
}

export function GroupJoinNotFound({ token }: GroupJoinNotFoundProps) {
            ...
            <div className="w-full bg-login-bg rounded-xl border border-login-border px-4 py-4 text-left space-y-3">
              ...
              <ul className="space-y-2">
                {[
                  ...
                  <li
                    key={i}
                    className="flex items-start gap-2 text-xs text-login-muted font-sans"
                  >
                    <span className="mt-0.5 shrink-0 text-login-muted/50 select-none">
                      •
                    </span>
                    {reason}
                  </li>
