...
export function GroupInvite({ groupId }: GroupInviteProps) {
  const router = useRouter();
  const { data: tokenData, isLoading } = useInviteToken(groupId);

  const inviteLink = tokenData ? `${APP_URL}/join/${tokenData.token}` : '';
  const inviteCode = tokenData ? tokenData.token : '';
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="bg-login-card border border-login-border rounded-2xl shadow-md px-7 py-7 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              {isLoading ? (
                <div className="flex-1 h-9 rounded-md bg-login-input animate-pulse" />
              ) : (
                <Input
                  readOnly
                  value={inviteLink}
        ...
        <p className="text-center text-[11px] text-login-muted mt-6 leading-relaxed font-sans px-2">
          招待リンクは有効期限がありません。不要になった場合はグループ設定から無効にできます。
        </p>
      </div>
